...
const CHARACTER_APPEND_PROMPT =
  "纯白背景，全身正面站立形象，双手自然下垂至身体两侧。从左到右依次生成【正脸头肩特写，全身正面视图，全身侧面视图，全身背面视图】，生成在一张图上。四格保持角色外形服装完全一致。";
...
const FIELD_KEYS = ["hair", "face", "body", "upperClothing", "lowerClothing", "shoes", "accessories"] as const;

/** Serialize a character item to editable text format (includes style prefix & append prompt) */
function serializeCharacter(item: CharacterItem): string {
  const lines: string[] = [];
  ...
  lines.push(CHARACTER_APPEND_PROMPT);
  return lines.join("\n");
}

/** Parse editable text back to character item fields */
function parseCharacterText(text: string): Partial<CharacterItem> {
  const result: Partial<CharacterItem> = {};
  const lines = text.split("\n");
  const reverseLabels: Record<string, string> = {};
  for (const [k, v] of Object.entries(FIELD_LABELS)) {
    reverseLabels[v] = k;
  }
  // Reverse style prefix lookup
  ...
  for (const line of lines) {
    ...
    if (match) {
      ...
      if (label === "名称") {
        ...
        // Try to match the style prefix
        const matchedStyle = reverseStyle[value];
        if (matchedStyle) {
          result.styleType = matchedStyle;
        }
...
/** Build image generation prompt for a character item */
function buildCharacterPrompt(item: CharacterItem): string {
  const styleWord = STYLE_PREFIX[item.styleType];
  const descParts: string[] = [];
  if (item.hair) descParts.push(item.hair);
  if (item.face) descParts.push(item.face);
  if (item.body) descParts.push(item.body);
  if (item.upperClothing) descParts.push(item.upperClothing);
  if (item.lowerClothing) descParts.push(item.lowerClothing);
  if (item.shoes) descParts.push(item.shoes);
  if (item.accessories) descParts.push(item.accessories);

  return `${styleWord}，${descParts.join("，")}，${CHARACTER_APPEND_PROMPT}`;
}
...
export const CharacterNode = memo(function CharacterNode({ id, data, selected }: NodeProps) {
  ...
  const [selectedAspectRatio, setSelectedAspectRatio] = useState(nodeData.requestAspectRatio || "3:4");
  ...
  const updateItem = useCallback(
    (itemId: string, patch: Partial<CharacterItem>) => {
      ...
      const poll = async () => {
        ...
        try {
          ...
          if (status.result) {
            ...
            (async () => {
              try { let p = status.result!; if (p.startsWith("data:") || p.startsWith("http")) { try { const lp = await persistImageSource(p) as string; if (lp && lp !== p) { updateItem(ctx.itemId, { imageUrl: lp } as any); cs.updateNodeData(outputNodeId, { imageUrl: lp }); p = lp; } } catch (e) { console.error("[角色节点] 图片持久化失败:", e); } } try { const prep = await prepareNodeImageSource(p, 0) as any; cs.updateNodeData(outputNodeId, { imageWidth: prep.width, imageHeight: prep.height }); } catch (e) { console.error("[角色节点] 预览尺寸解析失败:", e); } } catch (e) { console.error("[角色节点] 结果回调失败:", e); }
            })();
            resolve(); return;
          }
  ...
  const commitEdit = useCallback(() => {
    if (editingItemId) {
      const parsed = parseCharacterText(editDraft);
      // Ensure styleType from the button state is applied (in case user didn't edit the text)
  ...
  const handleGenerateSingle = useCallback(
    async (item: CharacterItem) => {
      ...
      const prompt = buildCharacterPrompt(item);
      const aspectRatio = selectedAspectRatio === "auto" ? "3:4" : selectedAspectRatio;
                      ...
                      <div className="flex items-center gap-1 nodrag" style={{ flexShrink: 0 }}>
                        {STYLE_KEYS.map((sk) => (
                          <button
                            key={sk}
                            onClick={() => {
                              const patch: Partial<CharacterItem> = { styleType: sk };
                              if (item.rawText) {
