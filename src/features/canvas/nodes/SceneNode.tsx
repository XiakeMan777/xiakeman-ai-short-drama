...
import {
  ...
  SCENE_VIEW_ANGLE_PROMPTS,
  SCENE_VIEW_ANGLE_LABELS,
...
const VIEW_ANGLE_KEYS: SceneViewAngle[] = ["front", "side", "top", "panorama"];
...
const FIELD_KEYS = ["type", "environment", "lighting", "atmosphere"] as const;

/** Serialize a scene item to editable text format (includes style prefix) */
function serializeScene(item: SceneItem): string {
  const lines: string[] = [];
  lines.push(`风格：${STYLE_PREFIX[item.styleType]}`);
  lines.push(`名称：${item.name}`);
  lines.push(`视角：${SCENE_VIEW_ANGLE_LABELS[item.viewAngle]}`);
...
/** Parse editable text back to scene item fields */
function parseSceneText(text: string): Partial<SceneItem> {
  const result: Partial<SceneItem> = {};
  const lines = text.split("\n");
  const reverseLabels: Record<string, string> = {};
  for (const [k, v] of Object.entries(FIELD_LABELS)) {
    reverseLabels[v] = k;
  }
  const reverseViewAngles: Record<string, SceneViewAngle> = {};
  for (const va of VIEW_ANGLE_KEYS) {
    reverseViewAngles[SCENE_VIEW_ANGLE_LABELS[va]] = va;
  }
  ...
  for (const line of lines) {
    ...
    if (match) {
      ...
      } else if (label === "视角" && reverseViewAngles[value]) {
        result.viewAngle = reverseViewAngles[value];
...
/** Build image generation prompt for a scene item */
function buildScenePrompt(item: SceneItem): string {
  const styleWord = STYLE_PREFIX[item.styleType];
  const angleWord = SCENE_VIEW_ANGLE_PROMPTS[item.viewAngle];
  const descParts: string[] = [];
  if (item.type) descParts.push(item.type);
  if (item.environment) descParts.push(item.environment);
  if (item.lighting) descParts.push(item.lighting);
  if (item.atmosphere) descParts.push(item.atmosphere);

  return `${styleWord}，${angleWord}，${descParts.join("，")}`;
}

// ─── SceneNode Component ────────────────────────────────────────────────

export const SceneNode = memo(function SceneNode({ id, data, selected }: NodeProps) {
  ...
  const [selectedAspectRatio, setSelectedAspectRatio] = useState(nodeData.requestAspectRatio || "16:9");
  ...
  const [editingViewAngle, setEditingViewAngle] = useState<SceneViewAngle>("front");
  ...
  const updateItem = useCallback(
    (itemId: string, patch: Partial<SceneItem>) => {
      ...
      const poll = async () => {
        ...
        try {
          if (Date.now() - pollStart > forceTimeout) {
            ...
            cs.updateNodeData(outputNodeId, { isGenerating: false, generationError: "图片生成超时" });
            updateItem(ctx.itemId, { isGenerating: false });
            addToast("error", "图片生成超时，请重试");
            if (ctx.shouldUseCredits) { tauriAiGateway.refundImageCredits(jobId, ctx.providerId); }
            resolve(); return;
          }
          if (Date.now() - pollStart > MAX_POLL_TIME) {
            stopped = true; activeJobsRef.current.delete(jobId);
            const cs = useCanvasStore.getState();
            cs.updateNodeData(outputNodeId, { isGenerating: false, generationError: "图片生成超时（15分钟）" });
            updateItem(ctx.itemId, { isGenerating: false });
            addToast("error", "图片生成超时（15分钟），请重试");
            if (ctx.shouldUseCredits) { tauriAiGateway.refundImageCredits(jobId, ctx.providerId); }
            resolve(); return;
          }
          ...
          if (status.progress !== undefined && status.progress >= 0) {
            ...
            cs2.updateNodeData(outputNodeId, { progressPercent: Math.round(status.progress) } as any);
          }
            ...
            (async () => {
              try {
                ...
                if (p.startsWith("data:") || p.startsWith("http")) {
                  try { const lp = (await persistImageSource(p)) as string; if (lp && lp !== p) { updateItem(ctx.itemId, { imageUrl: lp } as any); cs.updateNodeData(outputNodeId, { imageUrl: lp }); p = lp; } } catch (e) { console.error("[场景节点] 图片持久化失败:", e); }
                }
                try { const prep = (await prepareNodeImageSource(p, 0)) as any; cs.updateNodeData(outputNodeId, { imageWidth: prep.width, imageHeight: prep.height }); } catch (e) { console.error("[场景节点] 预览尺寸解析失败:", e); }
              } catch (e) { console.error("[场景节点] 结果回调失败:", e); }
            })();
            resolve(); return;
          }
  ...
  useEffect(() => {
    ...
    import("@/features/canvas/compat/event").then(({ listen }) => {
      ...
      listen<{ percent: number; stage: string; message: string }>("jimeng-browser-progress", (event) => {
        ...
        // Update all active generating items
        const items = nodeData.items || [];
        items.forEach((item) => {
  ...
  const startEdit = useCallback((item: SceneItem) => {
    setEditingStyleType(item.styleType);
    setEditingViewAngle(item.viewAngle);
    // Use rawText if available (preserves user formatting), otherwise serialize
    setEditDraft(item.rawText || serializeScene(item));
  ...
  /** Handle view angle button click while editing — replace view angle line in textarea */
  const handleViewAngleChangeInEdit = useCallback((newAngle: SceneViewAngle) => {
    setEditingViewAngle(newAngle);
    setEditDraft((prev) => {
      const newLabel = SCENE_VIEW_ANGLE_LABELS[newAngle];
      return prev.replace(/^视角：.*$/m, `视角：${newLabel}`);
    });
  }, []);

  const commitEdit = useCallback(() => {
    if (editingItemId) {
      const parsed = parseSceneText(editDraft);
      parsed.styleType = editingStyleType;
      parsed.viewAngle = editingViewAngle;
    ...
    setEditDraft("");
  }, [editingItemId, editDraft, editingStyleType, editingViewAngle, updateItem]);
  ...
  const handleGenerateSingle = useCallback(
    async (item: SceneItem) => {
      ...
      const aspectRatio = selectedAspectRatio === "auto" ? "16:9" : selectedAspectRatio;
      ...
      try {
        ...
        // This avoids blocking the frontend on slow API responses.
        // Backend spawns a tokio task, we poll DB status every 2s.
        ...
        // Start polling — waits for completion (serial mode for handleGenerateAll)
        await pollJobUntilDone(jobId, outputNodeId, 90000, {
          itemId: item.id,
          itemName: item.name || "场景图",
          shouldUseCredits,
  ...
  const handleGenerateAll = useCallback(async () => {
    const items = nodeData.items || [];
    const toGenerate = items.filter((item: any) => !item.imageUrl);
    if (toGenerate.length === 0) return;
    // Batch submissions (3 at a time) instead of Promise.all to avoid
    // overwhelming API rate limits. The backend semaphore handles
    // concurrent generation, but submitting 6+ requests in the same
    // second can trigger 429 errors before the semaphore kicks in.
    const BATCH_SIZE = 3;
              ...
              >
                {editingItemId === item.id ? (
                  ...
                  <div>
                    ...
                    {/* View angle buttons in edit mode */}
                    <div className="flex items-center gap-1 nodrag" style={{ marginBottom: "6px" }}>
                      {VIEW_ANGLE_KEYS.map((va) => (
                        <button
                          key={va}
                          onClick={() => handleViewAngleChangeInEdit(va)}
                          style={{
                            padding: "2px 8px",
                            borderRadius: "4px",
                            border: editingViewAngle === va ? "2px solid var(--accent-btn)" : "1px solid var(--border)",
                            backgroundColor: editingViewAngle === va ? "var(--bg-hover)" : "transparent",
                            color: editingViewAngle === va ? "var(--accent-btn)" : "var(--text-muted)",
                            fontSize: "11px",
                            cursor: "pointer",
                            transition: "all 0.15s",
                            fontWeight: editingViewAngle === va ? 600 : 400,
                          }}
                        >
                          {SCENE_VIEW_ANGLE_LABELS[va]}
                        </button>
                      ))}
                    </div>
                    <textarea
                  ...
                  // ─── View Mode ───
                  <>
                    {/* Card header: name + style + view angle */}
                      ...
                      <div className="flex items-center gap-1 nodrag" style={{ flexShrink: 0 }}>
                        {STYLE_KEYS.map((sk) => (
                          <button
                            key={sk}
                            onClick={() => {
                              const patch: Partial<SceneItem> = { styleType: sk };
                              if (item.rawText) {
                    ...
                    <div className="flex items-center gap-1 nodrag" style={{ marginBottom: "6px" }}>
                      {VIEW_ANGLE_KEYS.map((va) => (
                        <button
                          key={va}
                          onClick={() => updateItem(item.id, { viewAngle: va })}
                          style={{
                            padding: "2px 8px",
                            borderRadius: "4px",
                            border: item.viewAngle === va ? "2px solid var(--accent-btn)" : "1px solid var(--border)",
                            backgroundColor: item.viewAngle === va ? "var(--bg-hover)" : "transparent",
                            color: item.viewAngle === va ? "var(--accent-btn)" : "var(--text-muted)",
                            fontSize: "11px",
                            cursor: "pointer",
                            transition: "all 0.15s",
                            fontWeight: item.viewAngle === va ? 600 : 400,
                          }}
                        >
                          {SCENE_VIEW_ANGLE_LABELS[va]}
                        </button>
            ...
            <div className="flex items-center gap-3" style={{ flex: 1, minWidth: 0 }}>
              ...
              <StylePresetSelector
                ...
                onApply={(preset) => {
                  ...
                  if (preset.imageSize) setSelectedSize(preset.imageSize as ImageSize);
                }}
                nodeType="image"
              />
              <div
